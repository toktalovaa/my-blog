...
const UpdateProduct =() => {
    ...
    const updateProduct =(e)=>{
        const data = {
            ...
            image:e.currentTarget.img_url.value,
    ...
    fetch(url,options)
        .then(response => {
            if(response.ok){
                toast.success("Товар успешно обнавлен");
            }else{
                toast.error("Ошибка.Попробуйте заново");
            }
        }) 
}
     ...
     return(
            ...
            <form onSubmit={updateProduct} className={s.form} action="javascript:void(0)">
                ...
                <div>
                    ...
                    <input type="text" name="img_url" defaultValue={goods.image}/>
                </div>
                <div>
